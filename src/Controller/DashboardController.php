...
public function view() {
  ...
  // We’ll detect the “active assignment” by querying storage_assignment
  // with the unit reference + status = Active.
  ...
  foreach ($units as $unit) {
    ...
    // Find current (active) assignment for this unit.
    $aq = $a_storage->getQuery()->accessCheck(TRUE);
    $aq->condition('field_storage_unit', $unit->id());
    if ($has_status) {
...
public function history() {
  ...
  foreach ($assignments as $a) {
    ...
    $start_raw = $a->get('field_storage_start_date')->value;
    $end_raw = $a->get('field_storage_end_date')->value;

    $start = $start_raw ? date('Y-m-d', (int) $start_raw) : '—';
    $end = $end_raw ? date('Y-m-d', (int) $end_raw) : '—';
